...
class DbDataOpr(object):
    ...
    def init_sql_arr(self):
        ...
        return s_sql_info                    
    def getFormatDate(self,dict):
        ''''''
        device_time = datetime.datetime.strptime(dict['BASE']['device_time'],'%Y%m%d %H:%M:%S')
        device_date = str(device_time).split(' ')
        return device_date[0]
    ...
    def sql_proc_entry(self,dict,flag):
        ...
        self.record_date = self.getFormatDate(dict)
