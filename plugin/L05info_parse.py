...
class L05infoParse(object):
    ...
    def format_dict_info(self,dict_info):
        new_dict ={}
        format_cb = {}
        format_cb['BASE'] = self.format_base_callback
        for key,value in dict_info.items():
            if key in format_cb:
                try:
                    new_dict[key] = format_cb[key](value)
                except:
                    print 'format callback failed,reason:',sys.exc_info()
                    raise
            else:
                new_dict[key] = value
        return new_dict
    def format_base_callback(self,dict_info):
        '''基本信息的转换'''
 #       print dict_info
        new_dict = {}
        base_key = {"dev_id":"设备ID","customer":"设备账户","hard_plat":"硬件平台","device_ip":"设备IP",\
            "crash_time":"最近宕机时间","run_time":"持续运行时间","device_time":"上报时间",\
            "device_plat":"设备硬件平台","storage_plat":"存储硬件类型","device_version":"版本号",\
            "record_date":"上报日期","flag":"存在异常"}
        for key ,value in dict_info.items():
            if key in base_key:
                new_dict[base_key[key]] = value
        return new_dict
  ...
  #      print 'dict......',dict_info
        ...
        dict_info = self.format_dict_info(dict_info)
        ...
        print new_dict_info
        try:
  #          print json.dumps(new_dict_info,ensure_ascii=False)
            send_post(new_dict_info)
            print '======================================'
        except :    
            print sys.exc_info(),'\r\n配置文件插入数据数据失败'
            return False
    ...
    def common_parse_func(self,cf,name):
        ...
        try:
           ...
           # if re.search('DCLOG_',name) is not None:
            ...
            common_st = "=="
        dst_list_st = []
        dst_list_st.append(common_st)
        common_last_st['all'] = common_st
        return common_last_st
    ...
    def proc_DCLOG_SIZE_info(self,cf,name):
        ...
        for i in range(1,cnt+1,1):
            ...
            print key_word
            last_string_info += key_word + ' ['+self.common_parse_func(cf,key_word)+']'
            print last_string_info

            '''
            dclog_size_info['日志上报日期'] = key_word
            dclog_size_info['日志详情'] = self.common_parse_func(cf,key_word)
            '''
            try:
                log_detail_dict[i] =  last_string_info
            except:
                print sys.exc_info()
    
        print log_detail_dict        
        return 	log_detail_dict
